...
def move_folders(args):
    """Post-hoc folder management moving shards from each node into one consolidated path"""
    import subprocess
    import glob

    path = "/data/lux70/data/pfam/compressed/j1v1wv6w/train"
    # path = "/data/lux70/data/pfam/compressed/j1v1wv6w/val"
    files = glob.glob(f"{path}/*.tar")
    files.sort()
    print(len(files))

    for i, shard_path in enumerate(files):
        command = ["mv", shard_path, f"{path}/shard{i:03d}.tar"]
        print(" ".join(command))
