...
class _ToH5:
    ...
    def __init__(
        ...
        if use_dataparallel:
            self.esmfold = torch.nn.DataParallel(self.esmfold)
            self.hourglass = torch.nn.DataParallel(self.hourglass)
    ...
    def make_h5_database(self, h5_path, dataloader):
        ...
        for result in batch_results:
            start_idx, end_idx = result.get()
            with cur_idx.get_lock():
                cur_idx.value = end_idx
...
class FastaToH5Clans(_ToH5):
    ...
    def process_batch(self, batch):
        ...
        return compressed, clan_idxs, sequences, compressed_lens
