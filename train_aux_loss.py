...
def main(args: K.config.TrainArgs):
    ...
    if args.seq_loss_weight > 0.0:
        sequence_constructor = K.proteins.LatentToSequence(accelerator.device)
        ...
        while True:
            for batch in train_dl:
                with accelerator.accumulate(model):
                    ...
                    if args.seq_loss_weight > 0.0:
                        seq_loss = sequence_constructor.to_sequence(
                            model_output, sequences, mask=mask
                        )
                        loss += args.seq_loss_weight * seq_loss.mean().item()
