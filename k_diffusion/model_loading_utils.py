...
def load_model(filename, use_ema=True) -> T.Tuple[torch.nn.Module, torch.nn.Module, ModelConfig]:
    ...
    model_config = OmegaConf.create(ckpt['config']['model_config'])
    inner_model = (
        make_model(model_config).eval().requires_grad_(False)
    )
    if use_ema:
