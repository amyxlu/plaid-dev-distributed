...
class DPM_Solver:
    def __init__(
        ...
        """
        self.model_forward_wrapper = model_forward_wrapper  # returns noise
    ...
    def noise_prediction_fn(self, x, t, x_self_cond=None, model_kwargs={}):
        """
    ...
    def data_prediction_fn(self, x, t, x_self_cond=None, model_kwargs={}):
        ...
        """
        noise = self.noise_prediction_fn(x, t, x_self_cond=x_self_cond, model_kwargs=model_kwargs)
    ...
    def model_fn(self, x, t, x_self_cond=None, model_kwargs={}):
        """
        Convert the model to the noise prediction model or the data prediction model.
        """
        if self.algorithm_type == "dpmsolver++":
            return self.data_prediction_fn(x, t, x_self_cond=x_self_cond, model_kwargs=model_kwargs)
        else:
            return self.noise_prediction_fn(x, t, x_self_cond=x_self_cond, model_kwargs=model_kwargs)
    ...
    def denoise_to_zero_fn(self, x, s, x_self_cond=None, model_kwargs={}):
        """
        Denoise at the final step, which is equivalent to solve the ODE from lambda_s to infty by first-order discretization.
        """
        return self.data_prediction_fn(x, s, x_self_cond=x_self_cond, model_kwargs=model_kwargs)

    def dpm_solver_first_update(
        self, x, s, t, x_self_cond=None, model_kwargs={}, model_s=None, return_intermediate=False
    ):
        """
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if model_s is None:
                model_s = self.model_fn(x, s, x_self_cond=x_self_cond, model_kwargs=model_kwargs)
    ...
    def singlestep_dpm_solver_second_update(
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            model_s1 = self.model_fn(x_s1, s1, x_self_cond=x_self_cond, model_kwargs=model_kwargs)
    ...
    def singlestep_dpm_solver_third_update(
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            model_s2 = self.model_fn(x_s2, s2, x_self_cond=x_self_cond, model_kwargs=model_kwargs)
            if solver_type == "dpmsolver":
                x_t = (
                    (sigma_t / sigma_s) * x
                    - (alpha_t * phi_1) * model_s
    ...
    def multistep_dpm_solver_update(
        self, x, model_prev_list, t_prev_list, t, order, x_self_cond=None, model_kwargs={}, solver_type="dpmsolver"
    ):
        """
        ...
        if order == 2:
            ...
            higher_update = (
                lambda x, s, t, **kwargs: self.singlestep_dpm_solver_second_update(
                    x, s, t, x_self_cond=x_self_cond, model_kwargs=model_kwargs, r1=r1, solver_type=solver_type, **kwargs
                )
            )
        elif order == 3:
            r1, r2 = 1.0 / 3.0, 2.0 / 3.0
            lower_update = lambda x, s, t: self.singlestep_dpm_solver_second_update(
                x, s, t, x_self_cond=x_self_cond, model_kwargs=model_kwargs, r1=r1, return_intermediate=True, solver_type=solver_type
            )
            higher_update = (
                lambda x, s, t, **kwargs: self.singlestep_dpm_solver_third_update(
                    x, s, t, x_self_cond=x_self_cond, model_kwargs=model_kwargs, r1=r1, r2=r2, solver_type=solver_type, **kwargs
                )
            )
        else:
            raise ValueError(
    ...
    def sample(
        ...
        with torch.no_grad():
            if method == "adaptive":
                ...
                t_prev_list = [t]
                x_self_cond = None
                model_prev_list = [self.model_fn(x, t, x_self_cond, model_kwargs)]
                ...
                for step in range(1, order):
                    ...
                    model_prev_list.append(
                        self.model_fn(x, t, x_self_cond, model_kwargs)
                    )

                    x_self_cond = x

                # Compute the remaining values by `order`-th order multistep DPM-Solver.
                for step in trange(order, steps + 1, desc=f"Order {order} to step {steps}"):
                    t = timesteps[step]
                    ...
                    if step < steps:
                        model_prev_list[-1] = self.model_fn(
                            x, t, x_self_cond, model_kwargs
                        )
