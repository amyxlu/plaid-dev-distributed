...
class FunctionOrganismDiffusion(L.LightningModule):
    def __init__(
        ...
        *,
        ...
        self.clip_fn = partial(torch.clamp, min = self.x_clip_val * -1, max = self.x_clip_val)
    ...
    def model_predictions(self, denoiser_kwargs: DenoiserKwargs, cond_scale: float, rescaled_phi: float = 0.7):
        """Calls the forward_with_cond_scale method of the model to get the model output with conditioning (no label dropout)."""
        model_output = self.model.forward_with_cond_scale(denoiser_kwargs, cond_scale, rescaled_phi)
    ...
    def p_mean_variance(self, denoiser_kwargs: DenoiserKwargs, cond_scale: float, rescaled_phi: float = 0.7, clip_denoised = True):
        x, t = denoiser_kwargs.x, denoiser_kwargs.t
    ...
    def p_sample(
        ...
        batched_function_idx = torch.full((x.shape[0],), function_idx, device = x.device, dtype = torch.long)
        batched_organism_idx = torch.full((x.shape[0],), organism_idx, device = x.device, dtype = torch.long)
        ...
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(denoiser_kwargs = denoiser_kwargs, cond_scale = cond_scale, rescaled_phi = rescaled_phi, clip_denoised = clip_denoised)
    ...
    def ddim_sample_loop(
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            batched_function_idx = torch.full((batch,), function_idx, device=device, dtype=torch.long)
            batched_organism_idx = torch.full((batch,), organism_idx, device=device, dtype=torch.long)
            x_self_cond = x_start if self.self_condition else None
            ...
            cur_sample = x_start * alpha_next.sqrt() + \
                  c * pred_noise + \
                  sigma * noise
    ...
    def sample(
        ...
        if isinstance(function_idx, int):
            function_idx = torch.full((n_samples,), function_idx, device=self.betas.device, dtype=torch.long)
        
        if isinstance(organism_idx, int):
            organism_idx = torch.full((n_samples,), organism_idx, device=self.betas.device, dtype=torch.long)
        ...
        return sample_fn(shape, function_idx, organism_idx, mask, cond_scale, rescaled_phi, clip_denoised, return_all_timesteps)
    ...
    def run_step(
        self,
        batch: T.Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor],
